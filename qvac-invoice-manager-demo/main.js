...
let win = null;
let store = null;
let extractor = new Extractor();   // re-seeded from the store once it has loaded
...
// ---------------------------------------------------------------- model settings
// The SDK is only reachable through the extractor's cached import, so borrow it rather than opening
// a second one: two imports mean two registries.
const sdk = () => import("@qvac/sdk");

ipcMain.handle("models", async () => {
  const cat = await models.catalogue(sdk);
  const chosen = store.models();
  return {
    ...cat,
    selected: {
      text: models.textModel(chosen.text).key,
      vision: models.visionModel(chosen.vision).key,
    },
    cacheDir: models.CACHE_DIR,
  };
});

ipcMain.handle("set-models", async (_e, sel) => {
  refuseWhileBusy("change model");
  const saved = store.setModels({
    text: models.textModel(sel && sel.text).key,
    vision: models.visionModel(sel && sel.vision).key,
  });
  // Unload whatever is resident for a slot that changed, or the next document is silently read by
  // the model the user just deselected.
  await extractor.use({ textKey: saved.text, visionKey: saved.vision });
  send("models-changed", saved);
  return saved;
});

ipcMain.handle("download-model", async (_e, { kind, key }) => {
  refuseWhileBusy("download a model");
  const entry = kind === "text" ? models.textModel(key) : models.visionModel(key);
  busy = true; send("busy", true);
  try {
    const r = await models.download(sdk, entry, (p) => send("download-progress", p));
    return { ok: true, label: entry.label, ...r };
  } catch (e) {
    return { error: String((e && e.message) || e) };
  } finally {
    busy = false; send("busy", false);
  }
});
...
app.whenReady().then(() => {
  ...
  const chosen = store.models();
  extractor = new Extractor({ textKey: chosen.text, visionKey: chosen.vision });
  extractor.onProgress = (p) => send("download-progress", p);
  createWindow();
