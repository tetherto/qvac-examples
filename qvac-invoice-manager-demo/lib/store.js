...
class Store {
  constructor(dir) {
    ...
    this.data = { templates: [], rows: [], activeTemplateId: null, models: {} };
    this.load();
  }
  ...
  // ---- model choice ----
  // Stored by key, never by SDK constant name: a constant that gets renamed in a later SDK would
  // otherwise leave the app pointing at nothing, whereas an unknown key falls back to the default.
  models() { return this.data.models || {}; }

  setModels(sel) {
    this.data.models = { ...this.models(), ...sel };
    this.save();
    return this.models();
  }
